...
| `ui-enhancer-radar` | Visual quality. 14 domains including iPad sheet sizing (audits caller-side `.sheet(...)` for missing `.presentationSizing(.page)` / `.presentationDetents([.large])` / project convenience modifiers), Button hit region (three-factor detector for `.buttonStyle(.plain)` + trailing chevron + Form/List context — the combination that collapses tap targets on iPad), Silent Picker (menu-style `Picker` with no `.pickerStyle` in a custom non-Form container whose menu never presents on tap), color contrast, spacing, typography. |
