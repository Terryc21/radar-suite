...
| **ui-enhancer-radar** | Visual quality — requires you to view each screen before changes, walks through recommendations collaboratively, then finds similar patterns across views. 12 domains including iPad sheet sizing (caller-side `.sheet()` audit that flags tall content missing `.presentationSizing(.page)` / `.presentationDetents([.large])` / project convenience modifiers). |
